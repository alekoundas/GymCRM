...
import DataTableComponent from "../../core/datatable/DataTableComponent";
...
export default function TrainGroupDateGrid({ formMode }: IField) {
          ...
          return (
            <Dropdown
              ...
              onChange={(e: any) => {
                ...
                const rowIndex = editingRows.indexOf(row);
            ...
            //     // Update row data value and clear fields that should not have a value
            //     const trainGroupDates = trainGroupDto.trainGroupDates;
            //     let trainGroupDate = trainGroupDates.filter(
            //       (x) => x.id === rowId
            //     )[0];
            ...
            //       fixedDay:
            //         e.value === RecurringTrainGroupTypeEnum.FIXED_DAY
            //           ? trainGroupDate.fixedDay
            //           : undefined,
            //       recurrenceDayOfWeek:
            //         e.value === RecurringTrainGroupTypeEnum.DAY_OF_WEEK
            //           ? trainGroupDate.recurrenceDayOfWeek
            //           : undefined,
            //       recurrenceDayOfMonth:
            //         e.value === RecurringTrainGroupTypeEnum.DAY_OF_MONTH
            //           ? trainGroupDate.recurrenceDayOfMonth
            //           : undefined,
            //     };
            //     trainGroupDates.filter((x) => x.id === rowId)[0] =
            //       trainGroupDate;

            //     setTrainGroupDto({
            //       ...trainGroupDto,
            //       trainGroupDates: trainGroupDates,
            //     });
            //   }}
            //   options={Object.keys(RecurringTrainGroupTypeEnum)}
            //   optionLabel="type"
  ...
  const onRowEditInit = (event: { data: TrainGroupDateDto; index: number }) => {
    editingRows?.push({
      rowId: event.data.id,
      type: RecurringTrainGroupTypeEnum.FIXED_DAY,
    });
